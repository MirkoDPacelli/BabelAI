...
public class Main_V6 {
    ...
    public static void main(String[] args) {
        ...
        if (inputValue < 2 || inputValue > 14)
            ...
            BabelGA_V6.WORD_LENGTH = inputValue;

        //Start
        Dictionary_V6.createDictionary();
        startTime = System.currentTimeMillis();
        List<String> population = BabelGA_V6.generateFirstPopulation();

        for (; CURRENT_GENERATION < BabelGA_V6.MAX_GENERATIONS && CONTROL; CURRENT_GENERATION++) {
            //Evaluation population's fitness values
            for(int i = 0; i < BabelGA_V6.POPULATION_SIZE; i++) {
                BabelGA_V6.calculateFitness(population.get(i));
            ...
            if (CONTROL) { //Real english word not found
                ...
                population = BabelGA_V6.generateNextGeneration(BabelGA_V6.tournamentSelection(population));

                //Print results of current generation
                System.out.print("Generation " + (CURRENT_GENERATION + 1) + ": [");
                for (int j = 0; j < BabelGA_V6.POPULATION_SIZE-1; j++) {
                    System.out.print(population.get(j) + ", ");
                }
                System.out.println(population.get(BabelGA_V6.POPULATION_SIZE-1) + "]");
            }
        }
        exit(0);
    }
