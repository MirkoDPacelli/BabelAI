...
public class Main_V4 {
    ...
    public static void main(String[] args) {
        ...
        if (inputValue < 2 || inputValue > 14)
            ...
            BabelGA_V4.WORD_LENGTH = inputValue;

        //Start
        Dictionary_V4.createDictionary();
        startTime = System.currentTimeMillis();
        List<String> population = BabelGA_V4.generateFirstPopulation();

        for (; CURRENT_GENERATION < BabelGA_V4.MAX_GENERATIONS && CONTROL; CURRENT_GENERATION++) {
            //Evaluation population's fitness values
            for(int i = 0; i < BabelGA_V4.POPULATION_SIZE; i++) {
                BabelGA_V4.calculateFitness(population.get(i));
            ...
            if (CONTROL) { //Real english word not found
                ...
                population = BabelGA_V4.generateNextGeneration(BabelGA_V4.tournamentSelection(population));

                //Print results of current generation
                System.out.print("Generation " + (CURRENT_GENERATION + 1) + ": [");
                for (int j = 0; j < BabelGA_V4.POPULATION_SIZE-1; j++) {
                    System.out.print(population.get(j) + ", ");
                }
                System.out.println(population.get(BabelGA_V4.POPULATION_SIZE-1) + "]");
            }
        }
        exit(0);
    }
