package BabelAI_Version_6;

import java.util.ArrayList;
import java.util.List;
import java.util.Random;

public class BabelGA_V6 {
    ...
    public static int calculateFitness(String word) {
        int fitness = 0;
        for (String dictionaryWord : Dictionary_V6.dictionary) {
            ...
            if (match == length) {
                Main_V6.endTime = System.currentTimeMillis();
                System.out.println("\nFound real english word '" + word + "' at generation " + Main_V6.CURRENT_GENERATION + " in " + (Main_V6.endTime - Main_V6.startTime) + " ms");
                Main_V6.CONTROL = false;
            }
    ...
    public static List<String> tournamentSelection(List<String> population) {
        ...
        for (int i = 0; i < POPULATION_SIZE; i++) {
            ...
            String winner = rouletteWheelSelection(tournamentParticipants);
            selectionPopulation.add(winner);
        }
        return selectionPopulation;
    }

    private static String rouletteWheelSelection(List<String> population) {
        List<Integer> wheel = new ArrayList<>();
        int totalFitnessPopulation = 0;
        int populationSize = population.size();
        //Calculate fitness for each individual and total fitness of population
        for (int i = 0; i < (populationSize-1); i++) {
            String individual = population.get(i);
            int fitnessIndividual = calculateFitness(individual);
            totalFitnessPopulation += fitnessIndividual;
            wheel.add(i, totalFitnessPopulation);
        }

        int randomFitness = random.nextInt(totalFitnessPopulation) + 1;
        for (int j = 0; j < (populationSize-1); j++) {
            if (randomFitness <= wheel.get(j))
